...
def main(config_path: str, site: str, save_scores: bool, module: str|None=None):
    ...
    with open(os.path.join(mdir, f"{prefix}_columns.json"),'r') as f: columns = json.load(f)
    ...
    vae = rebuild_vae(len(columns), cfg['model']['hidden_dim'], cfg['model']['latent_dim'], weights)
    ...
    if save_scores:
        ...
        outp = os.path.join(cfg['preprocess']['out_dir'], 'scores', f"{tag}_scores.parquet")
