...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for a,b in [('DHI','GHI'), ('DNI','GHI'), ('POA','GHI')]:
        if a in out.columns and b in out.columns:
            out[f'ratio_{a}_{b}'] = np.where(out[b]>1e-3, out[a]/out[b], np.nan)
...
def select_training_matrix(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    drop_cols = [c for c in ['Pm','Pm_calc','Imp','Vmp','Isc','Voc'] if c in num.columns]
    X = num.drop(columns=drop_cols, errors='ignore').dropna()
    return df.loc[X.index], X

def enrich_with_module_labels(df: pd.DataFrame, module_map_path: str|None=None) -> pd.DataFrame:
    """Attach per-module labels (Sandia a,b; Character alpha,beta,gamma) as columns.
