...
def build_module_map_from_files(sandia_xlsx: str, characterization_xlsx: str) -> Dict[str, Any]:
    ...
    try:
        ...
        name_col = next((c for c in ['Name','Model','Module','Module Name','NREL identifier','Identifier','ID'] if c in df.columns), df.columns[0])
    ...
    try:
        ...
        for sh in xls.sheet_names:
            ...
            out['character'][_norm(sh)] = {'raw_sheet': sh, 'alpha': alpha, 'beta': beta, 'gamma': gamma}
    except Exception:
        pass
    return out
...
def choose_record(module_name: str, module_map: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {'sandia': module_map.get('sandia',{}).get(key), 'character': module_map.get('character',{}).get(key)}
